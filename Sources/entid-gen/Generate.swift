...
/// Reads `entid-rules.binpb`, applies the twenty five load checks and
...
/// of one.
///
///     entid-gen --rules spec/entid-rules.binpb \
///                    --lock rules.lock \
///                    --out Sources/EntID/Generated \
///                    [--check]
///
...
struct Generate {
    static func main() {
        do {
            ...
            FileHandle.standardError.write(Data("entid-gen: \(error.message)\n".utf8))
            exit(1)
        } catch {
            FileHandle.standardError.write(Data("entid-gen: \(error)\n".utf8))
            exit(1)
        }
    }
    ...
    static func run(_ arguments: [String]) throws {
        ...
        var outputPath = "Sources/EntID/Generated"
        var checkOnly = false
        ...
        if checkOnly {
            ...
            print("entid-gen: generated code matches \(lock.rulesVersion)")
            return
        }
        ...
        print("entid-gen: rules \(bundle.rulesVersion), format \(bundle.formatVersion)")
        print("entid-gen: \(bundle.definitions.count) definitions, \(bundle.dispatchers.count) kinds")
        print(
            "entid-gen: capabilities \(bundle.requiredFeatures.map(String.init).joined(separator: " "))")
        print("entid-gen: expansion \(bundle.expansion.summary)")
        print("entid-gen: wrote \(files.map(\.name).joined(separator: ", ")) to \(outputPath)")
    }
    ...
    static let usage = """
        entid-gen — compiles a EntID rule bundle to Swift.

          --rules <path>   the bundle to read (default spec/entid-rules.binpb)
          --lock <path>    the lock attesting it (default rules.lock)
          --out <path>     where to write (default Sources/EntID/Generated)
