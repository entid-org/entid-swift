...
///
///     businessid-fuzz [--seed N] [--rounds N] [--rules path]
@main
struct Fuzz {
    static func main() {
        var seed: UInt64 = 1
        ...
        print("businessid-fuzz: seed \(seed), \(rounds) rounds")
        ...
        if let bundle = FileManager.default.contents(atPath: rulesPath) {
            ...
            print("businessid-fuzz: no bundle at \(rulesPath), decoder rounds skipped")
        }
        fuzzDecoderFromNothing(&rng, rounds: rounds / 10)
        print("businessid-fuzz: no crash, no hang, no unbounded allocation")
    }
    ...
    static func fuzzEngine(_ rng: inout Rng, rounds: Int) {
        ...
        print("businessid-fuzz: \(reports) engine rounds")
    }
    ...
    static func fuzzDecoder(_ rng: inout Rng, seed: [UInt8], rounds: Int) {
        ...
        print("businessid-fuzz: \(rounds) mutated bundles, \(refused) refused, \(accepted) accepted")
    }
    ...
    static func fuzzDecoderFromNothing(_ rng: inout Rng, rounds: Int) {
        ...
        print("businessid-fuzz: \(rounds) arbitrary payloads, all refused")
    }
}
