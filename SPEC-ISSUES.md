...
### Section 11.4 step 4 does not reach schema-derived code — [spec#94](https://github.com/entid-org/spec/issues/94)

Step 4 says "régénère le code émis" and step 3 lists what a synchronization
writes. Neither reaches the code `protoc` emits from `rules.proto`,
`conformance.proto` and `testee.proto`, nor the package shaped tree `protoc`
needs it laid out in. `2026.08.38` moved the Protobuf package from
`libbusinessid.*` to `entid.*`, so both had to move, and section 11.4 gives a
synchronization no reason to touch either.

**Measured here.** Reproduced the tree the synchronization produces before any
hand step — new `spec/`, new `proto/entid/**`, new `rules.lock`, emitted rule
code regenerated by step 4, and the committed `*.pb.swift` still generated from
`libbusinessid.ir.v1`:
...
The section 12.6 entry point is green on wire code generated from a schema the
release replaced — soundly, since a package rename does not change the wire
format. The `Protobuf` job would catch it, and it does not run on a pull request
opened with `GITHUB_TOKEN`, so the drift would merge on green.

**Followed here:** `Tools/rules-sync.sh` writes the `proto/` tree itself, at the
path the `package` declaration of the attested schema names, and
`Tools/verify-lock.sh` derives the same path instead of holding a literal — the
literal is what refused this release at the end of an otherwise clean
synchronization. Regenerating the `.pb.swift` still needs `protoc`, which the
synchronization runner does not have; it was run by hand for this release, and
the reading that would make it automatic is not in the text yet.
...
### `spec/PROVENANCE.md` is the one file of step 3 the release does not carry — settled in `2026.08.38`

Step 3 made the engine write `spec/PROVENANCE.md` while every other file it
names was a release artefact. That one was not, and neither was anything needed
to build it: its single writer, `tools/write_provenance.sh`, reads
`docs/spec/provenance/body.md`, `docs/spec/provenance/<engine>.md`,
`docs/generated/coverage.md` and `go run ./cmd/entidc inspect`, all of which live
in the `spec` checkout. A workflow that had just verified a release still had to
clone `spec` beside it, pinned only by the `sourceCommit` the attested manifest
names.

Section 11.4 step 3 now says the provenance note is published assembled, one per
engine, so that "a moteur qui a vérifié une release n'a rien à cloner pour écrire
le dernier fichier de sa synchronisation". `v2026.08.38` publishes
`provenance-swift.md`, listed in `SHA256SUMS` and attested on its own.

**Measured before the clone was removed:** on `v2026.08.38`, running the pinned
`write_provenance.sh` and copying the published asset produce byte identical
files. `Tools/rules-sync.sh` now copies the asset, and the synchronization no
longer clones anything.

### `engine.md` numbered two different sections 12.5 — settled in `2026.08.38`

`### 12.5 Mutation testing` and `## 12.5 Une seule commande, silencieuse quand
tout passe` both carried 12.5, and the second sat at `##`, a sibling of `## 12.
Exigences qualité` rather than one of its subsections.

The single-command section is now `### 12.6`, at the heading level of its
siblings. Every citation in this repository that meant the entry point — the
`Makefile`, `Tools/verify.sh`, `Tools/rules-sync.sh`, `CLAUDE.md`,
`CONTRIBUTING.md` and both workflows — now says 12.6. `Tools/mutation.sh` still
says 12.5, and still means mutation testing.
