...
struct ConcurrencyTests {
    ...
    func concurrentValidate() async {
        let expected = Self.inputs.map { EntIDEngine.default.validate($0) }

        let observed = await withTaskGroup(of: (Int, ValidationReport).self) { group in
            for round in 0..<256 {
                let index = round % Self.inputs.count
                group.addTask {
                    (index, EntIDEngine.default.validate(Self.inputs[index]))
                }
            }
    ...
    func concurrentCanonicalize() async {
        let expected = Self.inputs.map { EntIDEngine.default.canonicalize($0) }

        let observed = await withTaskGroup(of: (Int, CanonicalizationResult).self) { group in
            for round in 0..<256 {
                let index = round % Self.inputs.count
                group.addTask { (index, EntIDEngine.default.canonicalize(Self.inputs[index])) }
            }
    ...
    func freshEnginesAreEquivalent() async {
        let shared = Self.inputs.map { EntIDEngine.default.validate($0) }
        let fresh = await withTaskGroup(of: [ValidationReport].self) { group in
            for _ in 0..<16 {
                group.addTask {
                    let engine = EntIDEngine()
