...
/// `Sources/EntIDTestee` or `Sources/entid-testee` can reach it.
enum TesteeHarness {
    static let root: URL = {
    ...
    struct TesteeUnavailable: Error, CustomStringConvertible {
        var description: String {
            "entid-testee was not found in .build; build it or set ENTID_TESTEE"
        }
    }
    ...
    static func executable() throws -> URL {
        if let override = ProcessInfo.processInfo.environment["ENTID_TESTEE"] {
        ...
        for triple in (try? manager.contentsOfDirectory(at: build, includingPropertiesForKeys: nil)) ?? [] {
            for configuration in ["debug", "release"] {
                let candidate = triple.appending(path: configuration).appending(path: "entid-testee")
