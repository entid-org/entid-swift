...
///
/// It rests on `spec/entid-conformance.jsonl`, which `rules.lock` attests
...
struct DocumentedValuesTests {
    static let documents = [
        "README.md",
        "Examples/EntIDConsumer/Sources/EntIDConsumer/main.swift",
    ]
    ...
    static func corpus() throws -> Corpus {
        let url = SpecCorpus.root.appending(path: "spec/entid-conformance.jsonl")
    ...
    func citedCasesExist(_ document: String) throws {
        ...
        for cited in try Self.matches(Self.caseIDPattern, in: try Self.text(document)) {
            ...
            // `entid-gen` are not case ids and must not be looked up.
